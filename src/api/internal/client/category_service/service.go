...
type CategoryService interface {
	...
	Update(ctx context.Context, id string, userID string, dto UpdateCategoryDTO) error
...
func (c *client) Update(ctx context.Context, id string, userID string, dto UpdateCategoryDTO) error {
	// TODO: check board permissions
...
func (c *client) UpdatePosition(ctx context.Context, dto UpdateCategoryListPositionDTO) error {
	...
	for _, item := range list.Categories {
		err = c.Update(ctx, item.ID, dto.UserID, UpdateCategoryDTO{Name: item.Name, Position: data[item.ID]})
		if err != nil {
