...
final class Kernel
{
    ...
    {
        $this->name = $_SERVER['APP_NAME'] ?? 'App';
    ...
    public static function run(bool $console = false): void
    {
        ...
        try {
            ...
            $status = $run(new Kernel($projectDir, $_SERVER['APP_ENV'] ?? 'dev', (bool)($_SERVER['APP_DEBUG'] ?? '0')));
        } catch (Throwable $e) {
     ...
     * @throws RuntimeException
     */
    private static function isWritable(string $dir): void
    {
        if (!is_writable($dir)) {
